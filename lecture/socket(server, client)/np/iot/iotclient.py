...
class IoTClient:
    ...
    def select_periodic(self, interval):
        ...
        if timeout_left > 0:
            events = self.sel.select(timeout=timeout_left) #5초에서 남은 시간을 찾기위해
    ...
    def run(self):
        ...
        while True:
            try:
                events = self.select_periodic(interval=5) #5초마다 읽어드리는 것
                if not events:      # timeout occurs
                    try:
                        temperature = next(gen_temp) #generator object라서 
                    ...
                    response_bytes = self.rfile.readline()     # receive response(response 메세지를 읽는 것)
                    ...
                    msgid = response.get('msgid')#response message에 msgid가 있으면 꺼내는 것
